     ...
     $                      u_vertdispl,buffvertdispl,done)
c********************************************************************
...
c     Vertex displacement info
      read(lun,'(a)',end=99,err=99) buffvertdispl(1)
      if(buffvertdispl(1).ne.'<vert_displacement>') then
         buffvertdispl(1)=' '
         backspace(lun)
         u_vertdispl=.false.
      else
         i=1
         do while(buffvertdispl(i).ne.'</vert_displacement>')
            i=i+1
            read(lun,'(a)',end=99,err=99) buffvertdispl(i)
         enddo
         u_vertdispl=.true.
      endif
...
c     Clustering info
 ...
 55   format(i3,5e19.11)         
     ...
     &     ievent,scale,aqcd, aqed,buff,u_syst,s_buff,nclus,buffclus,u_vertdispl,buffvertdispl)
c********************************************************************
...
c      
      ...
      integer nclus
      character*(clus_bufflen) buffclus(*)
c     to store info on the interaction point of long-lived DM particles 
      logical u_vertdispl
      character*30 buffvertdispl(*)
c
c     Local
...
c      aqcd = g*g/4d0/pi
     ...
     $                                              bias_weight,"</wgt>"
      ...
      if(u_vertdispl) then
         do i=1,3
            write(largeBuff,'(a)') buffvertdispl(i)(1:len_trim(buffvertdispl(i)))
         enddo
      endif
      if(u_syst)then
         do i=1,7
 ...
 51   format(i9,5i5,5e19.11,f3.0,f4.0)
     ...
     $     aqed,buff,u_syst,s_buff,nclus,buffclus,u_vertdispl,buffvertdispl)
c********************************************************************
c
...
c      aqcd = g*g/4d0/pi
     ...
     $                                                          "</wgt>"
      ...
      if(u_vertdispl) then
         do i=1,3
            write(lun,'(a)') buffvertdispl(i)(1:len_trim(buffvertdispl(i)))
         enddo
      endif
      if(u_syst)then
         do i=1,7
